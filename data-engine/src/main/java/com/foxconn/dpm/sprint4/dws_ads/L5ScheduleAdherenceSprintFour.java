...
/**
 * @author HS
 * @className L5ScheduleAdherenceSprintFour
 * @description TODO
 * @date 2020/5/15 14:53
 */
public class L5ScheduleAdherenceSprintFour extends DPSparkBase {
    ...
    public void calcluateDaySA() throws Exception {
        Dataset<Row> resultRows = sqlContext.sql(sqlGetter.Get("sprint_four_L5DaySvheduleAdherence.sql").replace("$ETL_TIME$", etl_time));
    ...
    public void calcluateWeekSA() throws Exception {
        Dataset<Row> resultRows = sqlContext.sql(sqlGetter.Get("sprint_four_L5WeekSvheduleAdherence.sql").replace("$ETL_TIME$", etl_time));
        resultRows.show();
    ...
    public void calcluateMonthSA() throws Exception {
        Dataset<Row> resultRows = sqlContext.sql(sqlGetter.Get("sprint_four_L5MonthSvheduleAdherence.sql").replace("$ETL_TIME$", etl_time));
        resultRows.show();
    ...
    public void calcluateQuarterSA() throws Exception {
        Dataset<Row> resultRows = sqlContext.sql(sqlGetter.Get("sprint_four_L5QuarterSvheduleAdherence.sql").replace("$ETL_TIME$", etl_time));
        resultRows.show();
    ...
    public void calcluateYearSA() throws Exception {
        Dataset<Row> resultRows = sqlContext.sql(sqlGetter.Get("sprint_four_L5YearSvheduleAdherence.sql").replace("$ETL_TIME$", etl_time));
        resultRows.show();
    ...
    public void getDWSProductionOutput(String yesterdayStamp, String todayStamp) throws Exception {
        ...
        JavaRDD<DpmDwsProductionOutputDD> dpmDwsProductionOutputDDJavaRDD1 = dpmDwsProductionOutputDD.filter(result -> {
            ...
            while (batchP.hasNext()) {
                ...
                if (batchGetter.getFilterRangeTimeStampHBeans(next, "DPM_DWS_PRODUCTION_OUTPUT_DD", "work_dt", "yyyy-MM-dd", Long.valueOf(yesterdayStamp), Long.valueOf(todayStamp))) {
                    ...
                    if (r.get(2).equals("L5") && r.get(14).equals("process") && r.get(4).matches("UpLoad_.+")) {
                        r.set(4, r.get(4).split("_")[1]);
                    }
                    if (r.get(2).equals("L5") && r.get(14).equals("line") ){
                        r.set(4, "Assy");
                    }
        ...
        try {
            for (DpmDwsProductionOutputDD dwsProductionOutputDD : dpmDwsProductionOutputDDJavaRDD.take(5)) {
                System.out.println(dwsProductionOutputDD.toString());
            }
        } catch (Exception e) {
