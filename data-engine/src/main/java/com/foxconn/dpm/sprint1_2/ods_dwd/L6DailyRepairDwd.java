...
/**
 * @version 1.0
 * @program: L6DailyRepairDwd
 * @description: L6 repair 数据插入（不分重庆武汉）
 * ODS層dpm_ods_production_repair_day_lten直接原樣轉到
 * DWD層dpm_dwd_production_repair_day_lten（bu_code轉成level_code成L16）
 * @author: xs
 * @create: 2020-07-14 14:00
 **/
public class L6DailyRepairDwd extends DPSparkBase {
    ...
    public void scheduling(Map<String, Object> map) throws Exception {
         ...
         * 获取维修信息,不加盐
         */
        JavaRDD<Result> repairData = DPHbase.saltRddRead("dpm_ods_production_repair_day_lsix", startRowkey, endRowkey, new Scan(), true);
        ...
        JavaRDD<ArrayList<String>> arrayListJavaRDD = repairData.mapPartitions(batchData -> {
            ...
            String family = "DPM_ODS_PRODUCTION_REPAIR_DAY_LSIX";
            ArrayList<ArrayList<String>> strings = new ArrayList<>();
            while (batchData.hasNext()) {
                Result output = batchData.next();
                SimpleDateFormat formatWorkDt = new SimpleDateFormat("yyyy-MM-dd");
                ArrayList<String> rs = beanGetter.resultGetConfDeftColumnsValues(output, "dpm_ods_production_repair_day_lsix", family);
                /**
                 * - site_code=String
                 * - area_code=String
                 * - line_code=String
                ...
                String rowkey = formatWorkDt.parse(rs.get(4)).getTime() + ":" + site_code + ":" + "L6" + ":" + rs.get(2) + ":" + rs.get(9) + ":" + UUID.randomUUID().toString().replace("-", "");
