 ...
 * @timestamp 2020/1/16
 */
public class UphDws extends DPSparkBase {
    ...
    public void scheduling(Map<String, Object> map) throws Exception {
        ...
        JavaRDD<UphPartnoOutput> mainRdd = pdOutputData.filter(r -> {
            ...
            return "line".equals(batchGetter.resultGetColumn(r, "DPM_DWS_PRODUCTION_OUTPUT_DD", "data_granularity"));
        }).keyBy(r -> {
        ...
        Dataset<Row> ds = session.sql(sqlGetter.Get("uph_dws.sql"));
        ds.show();
        ...
        try {
            ...
            List<Tuple2<String, Integer>> collect = ds.toJavaRDD().filter(r -> {
                return batchGetter.getStDateDayAdd(-1, "-").equals(r.getString(8));
            }).mapToPair(new PairFunction<Row, String, Integer>() {
                @Override
                public Tuple2<String, Integer> call(Row row) throws Exception {
                    return new Tuple2<String, Integer>(row.getString(8) + "_" + row.getString(1), Integer.valueOf(row.getString(10)));
                }
            }).reduceByKey((v1, v2) -> {
                return v1 + v2;
            }).collect();
        ...
        JavaRDD<Put> toWriteData = ds.toJavaRDD().mapPartitions(iterator -> {
            ...
            iterator.forEachRemaining(row -> {
                ...
                try {
                    ...
                    put.addColumn(Bytes.toBytes(family), Bytes.toBytes("data_granularity"), Bytes.toBytes("line"));
