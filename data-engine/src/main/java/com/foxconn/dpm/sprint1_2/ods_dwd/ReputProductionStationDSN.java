 ...
 */
public class ReputProductionStationDSN extends DPSparkBase {
    ...
    public void scheduling(Map<String, Object> map) throws Exception {
        ...
        if (dsn_target_day == null || "".equals(dsn_target_day)) {
            ...
            if (DSN_START_MIN == null || DSN_END_MIN == null) {
                ...
                DSN_END_MIN = String.valueOf(nowTimeDate.getTime());
            ...
            DSN_START_MIN = String.valueOf(format.parse(dsn_target_day + " 00:00:00.000").getTime());
            DSN_END_MIN = String.valueOf(format.parse(dsn_target_day + " 23:59:59.999").getTime() + 1);
        }
...
/*        DSN_START_MIN = "1590940800000";
    ...
    public void dpm_ods_production_output_day_lten() throws Exception {
         ...
         */
        BatchGetter batchGetter = MetaGetter.getBatchGetter();
        JavaRDD<ArrayList<String>> filter = DPHbase.saltRddRead("dpm_ods_production_output_day_lten", DSN_START_MIN, DSN_END_MIN, new Scan(), true).keyBy(r -> {
            return r.hashCode();
        }).partitionBy(new Partitioner() {
            @Override
            public int getPartition(Object key) {
                return ((Integer) key) % 600;
            }
        ...
        try {
            for (ArrayList<String> list : filter.take(5)) {
        ...
        try {
            List<Put> take = dpm_ods_production_output_day_lten_Puts.take(5);
            for (Put put : take) {
    ...
    public void dpm_ods_production_output_lsix_day() throws Exception {
         ...
         */
        //读取产量数据
        JavaRDD<ArrayList<String>> filter = DPHbase.saltRddRead("dpm_ods_production_output_day_lsix", DSN_START_MIN, DSN_END_MIN, new Scan(), true).keyBy(r -> {
            return r.hashCode();
        }).partitionBy(new Partitioner() {
            @Override
            public int getPartition(Object key) {
        ...
        try {
            List<Put> take = dpm_ods_production_output_day_lsix_Puts.take(5);
            for (Put put : take) {
                System.out.println(put);
            }
        } catch (Exception e) {
    ...
    public void dpm_ods_production_output_lfive_day() throws Exception {
        ...
        JavaRDD<ArrayList<String>> filter = DPHbase.saltRddRead("dpm_ods_production_output_day_lfive", DSN_START_MIN, DSN_END_MIN, new Scan(), true).keyBy(r -> {
            return r.hashCode();
        }).partitionBy(new Partitioner() {
            @Override
            public int getPartition(Object key) {
        ...
        try {
            List<Put> take = dpm_ods_production_output_day_lfive_Puts.take(5);
