...
--   nvl(cast(t4.uph_adherence_target AS FLOAT) * 100, 0)  uph_adherence_target,
  cast(
      ...
      ) AS INTEGER) uph_adherence_target,
    ...
    FROM
      (
        ...
        SELECT
          ...
          t1.line_code,
          t1.customer,
          t1.quarter_id                                                                           AS quarter_id,
          cast(nvl(bround(sum(nvl(c_output_qty, 0)) / (24*if(CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd')))), 4), 0) AS INTEGER)                   AS uph_actual,
          cast(nvl(sum(nvl(c_output_qty, 0)), 0) AS INTEGER)                                   AS output_qty_actual,
          cast(nvl(bround(3600 / bround(sum(nvl(c_output_qty, 0)) / (24*if(CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd')))), 4), 4), 0) AS INTEGER) AS cycle_time_actual,
          24*if(CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd')))                     AS output_hours,
          cast(
              nvl(
                  bround(
        ...
        FROM
          (
            ...
            GROUP BY site_code, level_code, line_code, cast(concat(year(work_dt), quarter(work_dt)) AS INTEGER), customer,area_code
          ) t1
          LEFT JOIN
          (
            SELECT
            ...
            GROUP BY site_code, level_code, line_code, cast(concat(year(work_dt), quarter(work_dt)) AS INTEGER),area_code,customer
          ) t2 ON t2.site_code = t1.site_code AND t2.level_code = t1.level_code AND t2.line_code = t1.line_code
                  AND t2.quarter_id = t1.quarter_id AND  t1.area_code = t2.area_code AND t1.customer = t2.customer
        GROUP BY
          t1.site_code, t1.level_code, t1.quarter_id, t1.customer, t1.process_code , t1.line_code
      ) t
    UNION ALL
    SELECT
      site_code,
