...
select
  ...
  sum(output_qty)/(24*if(CalculteYearDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteYearDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))))          uph_actual,
  (24*if(CalculteYearDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteYearDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))))      output_hours,
  cast(
...
where work_dt = year(from_unixtime((unix_timestamp()-(60*60*24)), 'yyyy-MM-dd'))
group by
  site_code,
  level_code,
  factory_code,
