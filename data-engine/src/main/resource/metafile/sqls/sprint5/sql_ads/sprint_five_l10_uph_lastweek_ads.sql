    ...
    FROM
      (
        ...
        SELECT
          ...
          cast(nvl(bround(sum(nvl(c_output_qty, 0)) / (24*7), 4), 0) AS INTEGER)                   AS uph_actual,
          cast(nvl(sum(nvl(c_output_qty, 0)), 0) AS INTEGER)                                   AS output_qty_actual,
          cast(nvl(bround(3600 / bround(sum(nvl(c_output_qty, 0)) / (24*7), 4), 4), 0) AS INTEGER) AS cycle_time_actual,
          24*7                   AS output_hours,
          cast(
              nvl(
                  bround(
        ...
        FROM
          ...
          (
            ...
            GROUP BY site_code, level_code, line_code, calculateYearWeek(work_dt),area_code,customer
                  ...
                  AND t2.week_id = t1.week_id AND  t1.area_code = t2.area_code AND t1.customer = t2.customer
        GROUP BY
