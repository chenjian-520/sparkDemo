...
select
  ...
  sum(output_qty)/(24*if(CalculteWeekDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteWeekDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))))          uph_actual,
  (24*if(CalculteWeekDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteWeekDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))))      output_hours,
  cast(
