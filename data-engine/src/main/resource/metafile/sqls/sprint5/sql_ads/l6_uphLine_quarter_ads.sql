...
select
  ...
  sum(output_qty)/(24*if(CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))))          uph_actual,
  (24*if(CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteQuarterDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))))      output_hours,
  cast(
