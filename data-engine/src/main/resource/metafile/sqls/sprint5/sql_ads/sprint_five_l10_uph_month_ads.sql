    ...
    FROM
      (
        ...
        SELECT
          ...
          cast(nvl(bround(sum(nvl(c_output_qty, 0)) / (24*if(CalculteMonthDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteMonthDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd')))), 4), 0) AS INTEGER)                   AS uph_actual,
          cast(nvl(sum(nvl(c_output_qty, 0)), 0) AS INTEGER)                                   AS output_qty_actual,
          cast(nvl(bround(3600 / bround(sum(nvl(c_output_qty, 0)) / (24*if(CalculteMonthDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteMonthDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd')))), 4), 4), 0) AS INTEGER) AS cycle_time_actual,
          24*if(CalculteMonthDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd'))=0,1,CalculteMonthDay(from_unixtime((unix_timestamp()), 'yyyy-MM-dd')))                     AS output_hours,
          cast(
              nvl(
                  bround(
        ...
        FROM
          ...
          (
            ...
            GROUP BY site_code, level_code, line_code, cast(from_unixtime(to_unix_timestamp(work_dt, 'yyyy-MM-dd'), 'yyyyMM') AS INTEGER),area_code,customer
        ...
        GROUP BY
          t1.site_code, t1.level_code, t1.month_id, t1.customer, t1.process_code ,t1.line_code
      ) t
    UNION ALL
    SELECT
      site_code,
