SELECT
  ...
  CAST(formatNumber(nvl(day_oee_fpy.time_efficiency_rate -( day_oee_fpy.performance_efficiency_rate + day_oee_fpy.bad_losses), 0))
...
FROM
  ...
  (
    SELECT
      calculateYearWeek(from_unixtime(to_unix_timestamp(L5_6_10.WorkDate, 'yyyyMMdd'), 'yyyy-MM-dd')) week_id,
      L5_6_10.Site_Code,
      L5_6_10.Level_Code,
      sum(L5_6_10.QTY)                                                                                QTY,
      CAST(nvl(max(L5_6_10.online_dl_upph_target), 0) AS
           VARCHAR(32))                                                                               online_dl_upph_target,
                     ...
                     nvl(
                         IF(dpm_dws_if_m_dop.WOType = 'BTO',
                            ...
                            IF(dpm_dws_if_m_dop.WOType = 'CTO',
                               dpm_dws_if_m_dop.QTY * dpm_ods_manual_normalization.
                               Normalization_CTO,
                               dpm_dws_if_m_dop.QTY *
    ...
    GROUP BY
      calculateYearWeek(from_unixtime(to_unix_timestamp(L5_6_10.WorkDate, 'yyyyMMdd'), 'yyyy-MM-dd')),
      L5_6_10.Level_Code,
      L5_6_10.Site_Code
  ) L5_6_10
  ...
  LEFT JOIN (
              ...
              SELECT
                calculateYearWeek(
                    from_unixtime(to_unix_timestamp(dpm_ods_manual_manhour.Date, 'yyyyMMdd'), 'yyyy-MM-dd')) week_id,
              ...
              GROUP BY
                calculateYearWeek(
                    from_unixtime(to_unix_timestamp(dpm_ods_manual_manhour.Date, 'yyyyMMdd'), 'yyyy-MM-dd')),
                dpm_ods_manual_manhour.Site,
                dpm_ods_manual_manhour.Level

            ) dpm_ods_manual_manhour ON
                                       dpm_ods_manual_manhour.week_id
                                       =
                                       L5_6_10.week_id
                                       AND
  ...
  LEFT JOIN (
              SELECT
                calculateYearWeek(
                    from_unixtime(to_unix_timestamp(ehr_man_hour.WorkDT, 'yyyyMMdd'), 'yyyy-MM-dd')) week_id,
              ...
              GROUP BY
                calculateYearWeek(from_unixtime(to_unix_timestamp(ehr_man_hour.WorkDT, 'yyyyMMdd'), 'yyyy-MM-dd')),
                ehr_man_hour.SiteCodeID,
                ehr_man_hour.LevelCodeID
            ) ehr_man_hour ON
                             ehr_man_hour.week_id
                             =
                             L5_6_10.week_id
                             AND
                             ehr_man_hour.SiteCodeID
  ...
  LEFT JOIN (
    ...
    ON ehr_man_hour_last_day_headcount.week_id = L5_6_10.week_id AND ehr_man_hour_last_day_headcount.r = 1
  ...
  LEFT JOIN (SELECT
               calculateYearWeek(
                   from_unixtime(to_unix_timestamp(day_oee_fpy.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')) week_id,
             ...
             GROUP BY
               calculateYearWeek(
                   from_unixtime(to_unix_timestamp(day_oee_fpy.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')),
               day_oee_fpy.site_code,
               day_oee_fpy.level_code

            ) day_oee_fpy ON
                            day_oee_fpy.week_id = L5_6_10.week_id
                            AND
                            day_oee_fpy.site_code
                            =
                            L5_6_10.Site_Code
