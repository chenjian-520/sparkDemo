SELECT
  ...
  ''                                                                             site_code_desc,
  ''                                                                             level_code_desc,
  CAST(formatNumber(L5_6_10.online_dl_upph_target) AS FLOAT) online_dl_upph_target,
  CAST(formatNumber(L5_6_10.offline_var_dl_upph_target) AS FLOAT)  offline_var_dl_upph_target,
  CAST(formatNumber(L5_6_10.offline_fix_dl_headcount_target) AS INTEGER) offline_fix_dl_headcount_target,
  '${etl_time}'                                                                  etl_time,
...
FROM
  ...
  LEFT JOIN (
              ...
              SELECT
                ...
                nvl(cast(sum(dpm_ods_manual_manhour.Output) AS FLOAT) , 0.0)       Output,
    ...
    LEFT JOIN (
      ...
      ON ehr_man_hour_last_day_headcount.quarter_id = L5_6_10.quarter_id AND ehr_man_hour_last_day_headcount.r = 1
    ...
    LEFT JOIN (SELECT
                 cast(concat(year(from_unixtime(to_unix_timestamp(day_oee_fpy.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')),quarter(from_unixtime(to_unix_timestamp(day_oee_fpy.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')))  AS INTEGER) quarter_id,
               ...
               FROM day_oee_fpy
               GROUP BY
                 cast(concat(year(from_unixtime(to_unix_timestamp(day_oee_fpy.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')),quarter(from_unixtime(to_unix_timestamp(day_oee_fpy.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')))  AS INTEGER),
                 day_oee_fpy.site_code,
                 day_oee_fpy.level_code

              ) day_oee_fpy ON
                              day_oee_fpy.quarter_id = L5_6_10.quarter_id
                              AND
                              day_oee_fpy.site_code
                              =
                              L5_6_10.Site_Code
