SELECT
  ...
  concat(ifnull(sum(output.normalized_output_qty) / sum(manhour.DL1_TTL_Manhour),
                0), ' ')                                 online_dl_upph_actual,
  concat(ifnull(sum(output.normalized_output_qty) / sum(manhour.DL2_Variable_Manhour), 0),
         ' ')                                            offline_var_dl_upph_actual,
  from_unixtime(to_unix_timestamp(output.WorkDT, 'yyyyMMdd'), 'yyyyMM')
                                                         month_id,
  (CASE
...
FROM dpm_dws_production_output_day output
  LEFT JOIN dpm_ods_manual_manhour manhour ON
                                             from_unixtime(to_unix_timestamp(manhour.Date, 'yyyyMMdd'), 'yyyyMM')
                                             =
                                             from_unixtime(to_unix_timestamp(output.WorkDT, 'yyyyMMdd'), 'yyyyMM')
