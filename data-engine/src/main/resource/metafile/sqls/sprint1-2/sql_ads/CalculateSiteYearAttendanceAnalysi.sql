SELECT
  ...
  now_week_total_production_site_data.year_id,
...
FROM
  (
    SELECT
      cast(year(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')) AS INTEGER) year_id,
    ...
    WHERE cast(year(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')) AS INTEGER) =
          cast(year(from_unixtime(to_unix_timestamp(now(), 'yyyyMMdd'), 'yyyy-MM-dd')) AS INTEGER)
...
FROM
  (
    ...
    FROM total_production_site_data
    WHERE
      cast(year(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')) AS INTEGER) =
      cast(year(from_unixtime(to_unix_timestamp(now(), 'yyyyMMdd'), 'yyyy-MM-dd')) AS INTEGER) - 1
    ORDER BY total_production_site_data.work_date DESC LIMIT 1
