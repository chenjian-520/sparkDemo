...
FROM (SELECT t4.site_code,
             t4.level_code,
             CAST(t4.quarter_id AS INTEGER)          quarter_id,
      ...
      FROM (SELECT t1.site_code,
            ...
            FROM (SELECT site_code,
                  ...
                  GROUP BY site_code, level_code,
                           cast(concat(year(work_dt), quarter(work_dt)) AS INTEGER)) t1
                   LEFT JOIN (SELECT site_code,
                              ...
                              GROUP BY site_code, level_code,
                                       cast(concat(year(work_dt), quarter(work_dt)) AS INTEGER)) t2
                   ...
                   LEFT JOIN (SELECT site_code,
                              ...
                              GROUP BY site_code, level_code,
                                       cast(concat(year(work_dt), quarter(work_dt)) AS INTEGER)) t3
                     ON t3.site_code = t1.site_code
                          AND t3.level_code = t1.level_code
                          AND t3.quarter_id = t1.quarter_id) t4
      GROUP BY t4.site_code, t4.level_code, t4.quarter_id, t4.ict_qty, t4.pass_qty, t4.fct_qty) t5
GROUP BY t5.site_code, t5.level_code, t5.quarter_id, t5.ict, t5.fct
