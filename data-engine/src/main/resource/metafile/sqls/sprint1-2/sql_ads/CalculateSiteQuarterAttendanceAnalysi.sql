SELECT
  ...
  now_week_total_production_site_data.quarter_id,
...
FROM
  (
    SELECT
      cast(concat(year(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')),quarter(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')))  AS INTEGER)       quarter_id,
    ...
    WHERE  cast(concat(year(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')),quarter(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')))  AS INTEGER) =
           cast(concat(year(from_unixtime(to_unix_timestamp(now(), 'yyyyMMdd'), 'yyyy-MM-dd')),quarter(from_unixtime(to_unix_timestamp(now(), 'yyyyMMdd'), 'yyyy-MM-dd')))  AS INTEGER) - 1
...
SELECT
  concat(unix_timestamp(), '-', uuid())                                           id,
  pre_week_total_production_site_data.quarter_id,
...
FROM
  (
    ...
    FROM total_production_site_data
    WHERE
      cast(concat(year(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')),quarter(from_unixtime(to_unix_timestamp(total_production_site_data.work_date, 'yyyyMMdd'), 'yyyy-MM-dd')))  AS INTEGER) =
      cast(concat(year(from_unixtime(to_unix_timestamp(now(), 'yyyyMMdd'), 'yyyy-MM-dd')),quarter(from_unixtime(to_unix_timestamp(now(), 'yyyyMMdd'), 'yyyy-MM-dd')))  AS INTEGER) - 1
    ORDER BY total_production_site_data.work_date DESC LIMIT 1
