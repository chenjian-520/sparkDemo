SELECT concat(unix_timestamp(), '-', uuid())       id,
       ...
       nvl(
         get_aim_target_by_key(concat_ws('=', 'D', t5.site_code, t5.level_code, 'all', 'all', 'all', 'all'), 16) * 100,
         0)                                        fpy_target,
       '$ETL_TIME$'                                etl_time
FROM (SELECT t4.site_code,
      ...
      GROUP BY t4.site_code, t4.level_code, t4.work_dt, t4.ict_qty, t4.pass_qty, t4.fct_qty) t5
GROUP BY t5.site_code, t5.level_code, t5.work_dt, t5.ict, t5.fct
