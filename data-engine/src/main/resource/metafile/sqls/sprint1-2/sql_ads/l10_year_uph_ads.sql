    ...
    FROM
      (
        SELECT
          ...
          sum(ct)         AS c_ct
        FROM uphPartnoDay
        GROUP BY site_code, level_code, line_code, year(work_dt),area_code
      ) t1
      LEFT JOIN
      (
        SELECT
        ...
        GROUP BY site_code, level_code, line_code, year(work_dt),area_code
              ...
              AND t2.year_id = t1.year_id AND t1.area_code = t2.area_code
    GROUP BY
      t1.site_code, t1.level_code, t1.year_id
