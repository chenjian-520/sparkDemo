SELECT
  ...
  formatNumber(nvl(cast(ehr_man_hour.DL2F / ehr_man_hour.counter AS INTEGER), 0)) offline_fix_dl_headcount_actual,
  0                                                                               site_code_id,
  0                                                                               level_code_id,
  ''                                                                              site_code_desc,
  ''                                                                              level_code_desc,
...
FROM
  ...
  LEFT JOIN (
              ...
              SELECT
                calculateYearWeek(from_unixtime(to_unix_timestamp(dpm_ods_manual_manhour.Date, 'yyyyMMdd'), 'yyyy-MM-dd')) week_id,
  ...
  LEFT JOIN (
              SELECT
                calculateYearWeek( from_unixtime(to_unix_timestamp(ehr_man_hour.WorkDT, 'yyyyMMdd'), 'yyyy-MM-dd')) week_id,
