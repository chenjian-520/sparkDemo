...
FROM
  (
    SELECT
      ...
      dpm_ods_production_planning_day_sc.work_dt                               work_date,
      ...
      '$ETL_TIME$'                                                             etl_time,
      dpm_ods_production_planning_day_sc.schedule_qty,
      dpm_dws_production_output_dd_mthine.output_qty,
    ...
    FROM
      ...
      LEFT JOIN (
        ...
        ON
          dpm_ods_production_planning_day_sc.work_dt = dpm_dws_production_output_dd_mthine.work_dt AND
    ...
    SELECT
      ...
      cast(nvl(get_aim_target_by_key(
                   concat_ws("=", 'D', dpm_ods_production_planning_day_sc.site_code,
                             dpm_ods_production_planning_day_sc.level_code, 'all', 'all', 'all', 'all'), 12
               ), 0) * 100 AS FLOAT)                                           schedule_adherence_target
    FROM
      (
        SELECT
