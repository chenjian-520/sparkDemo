    ...
    select
      ...
      from
        (
          ...
          from
            (select * from attendance_day where work_date != '#' and work_date != '##' and work_date != '###' and work_date != '####')
        ) as a

      group by a.site_code , a.level_code,a.factory_code,a.work_date
  ) as b
where b.week_id = calculateYearWeek(from_unixtime((unix_timestamp()-(60*60*24)), 'yyyy-MM-dd')) or b.week_id = calculateYearWeek(from_unixtime((unix_timestamp()-(60*60*24*8)), 'yyyy-MM-dd')) and site_code = 'WH'